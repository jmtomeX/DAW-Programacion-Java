...
Correcciones : Si las excepciones se gestionan con try-catch, ya no hay que lanzarlas con throws. Recuerda cerrar también el fichero de escritura.
                 En este caso, después de leer una línea no hay que detectar sus palabras por lo que no hay que convertirla a un Scanner para procesarla.
                 Tanto la linea de la descripción como la línea de la secuencia deberían ser Strings y podrías trabajar con ellas directamente.
                 Tampoco necesitás el contador y el if-else. Siempre se leen 2 líneas, primero la descripción que simplemente hay que escribir y después la secuencia que se procesará.
                 De esta manera además, podrías agrupar todos los println en el método escribirFichero y llamarlo 2 veces.
                 valoresNucleotidos es una constante a nivel de clase. No hay que pasarla como parámetro.
                 Dentro del if(lista.contains("-")), solo usas realmente la línea del replace.
                 No necesitas contar los codones. codones.length ya te dice cuántos hay. 
...
//Número mínimo de codones de una proteina 
 ...
 //Número de nucleótidos por codon
  ...
  //Muestra los demás resultados  
   public static void trabajarFicheros(Scanner teclado) throws FileNotFoundException {
      ...
      try{  //Si el fichero no se abre correctamente se producirá una excepción.
         ...
         while(leerFichero.hasNextLine()){
           ...
           //comprobamos que es la  línea par que es la última de cada grupo en el archivo.
            if(cont % 2 == 0){
               ...
               escribirFich.print("Nucleótidos: ");
         ...
         //mientras que tenga para leer palabras las leerá  
            ...
            //comprobamos que es la segunda línea de cada grupo de datos.
            if(cont % 2 == 0) {
               System.out.println("Contadores: "  + java.util.Arrays.toString(contadores));
               System.out.println("Masa (%): " + java.util.Arrays.toString(listadoMasasNucleotidos) + " de " +  redondearDecimales(masaTotal));
               System.out.println("Lista Codones: "  + java.util.Arrays.toString(listaCodones)); 
               System.out.println("Es proteina: " + confirmacionProteina);
               System.out.println();
               
               escribirFich.println("Contadores: "  + java.util.Arrays.toString(indiceNucl));
               escribirFich.println("Masa (%): " + java.util.Arrays.toString(listadoMasasNucleotidos) + " de " + masaTotal);
               escribirFich.println("Lista de codones: "  + java.util.Arrays.toString(listaCodones)); 
               escribirFich.println("Es proteina: " + confirmacionProteina);
               escribirFich.println();
            }         
         } 
         escribirFich.close();
